-- ============================================================
--  LuaMore · Neon Bar — wide in-game loading bar
--  Run as a LocalScript. Set LOADER_URL to the loader from
--  your LuaMore dashboard. A bottom-center pill with a cyan
--  glow line that reads clearly on bright game maps.
--
...
local LOADING_POSITION = "bottom-center" -- "top" | "bottom-center" | "bottom-right" | "bottom-left"
local LOADING_BAR_BG = Color3.fromRGB(6, 12, 20)
local LOADING_ACCENT = Color3.fromRGB(0, 229, 255)
local LOADING_TEXT = Color3.fromRGB(235, 252, 255)
local LOADING_SUBTEXT = Color3.fromRGB(120, 170, 185)
local LOADING_DIM = false
local LOADING_SCREEN_BG = Color3.fromRGB(6, 12, 20)
local STROKE_COLOR = Color3.fromRGB(0, 229, 255)
local STROKE_ALPHA = 0.55
local SHOW_STROKE = true
local TRACK_COLOR = Color3.fromRGB(10, 30, 40)
-- -----------------------------------------------------------
...
-- Neon decoration: a soft cyan glow frame tucked behind the bar,
-- centered regardless of which corner the bar is anchored to.
local function decorate(bar)
	local glow = Instance.new("Frame")
	glow.Name = "NeonGlow"
	glow.BackgroundColor3 = Color3.fromRGB(0, 229, 255)
	glow.BackgroundTransparency = 0.88
	glow.BorderSizePixel = 0
	glow.Size = bar.Size + UDim2.fromOffset(12, 12)
	glow.AnchorPoint = bar.AnchorPoint
	glow.Position = bar.Position + UDim2.fromOffset(
		(0.5 - bar.AnchorPoint.X) * -12,
		(0.5 - bar.AnchorPoint.Y) * -12
	)
	glow.ZIndex = -1
	glow.Parent = bar.Parent
	corner(glow, 16)
end
