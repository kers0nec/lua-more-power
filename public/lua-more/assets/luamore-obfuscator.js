...
`;return{tokens:u,decoderRuntime:g}}function Te(e){let n=v(1e3,9999),t=v(1e4,99999),r=v(1e5,999999),a=0;return`
local _lm_state = ${n}
...
`}function Ee(e,n){let t={},r=new Set;for(let h=1;h<=15;h++){let $=v(100,899);for(;r.has($);)$=v(100,899);r.add($),t[h]=$}let a=pe(e),{tokens:u}=be(a),{tokens:o,decoderRuntime:l}=ye(u),f="";for(let h of o)h.type!=="COMMENT"&&(f+=h.raw);l&&(f=l+`
`+f),(n.controlFlowFlattening??!0)&&(f=Te(f));let i=d(new Set),g=d(new Set),c=d(new Set),_=d(new Set);return`
...
local ${$}=(string and string.char) or ${_}(_G, ${C("string.char")})
  ...
  elseif ${m}==${ne} then
    ...
    while _pos<=#${k} do
      ...
      if _hdr>=128 then
        ...
        _pos=_pos+1
        if _byteVal then
          for _=1,_runLen do ${P}[_outPtr]=${$}(_byteVal); _outPtr=_outPtr+1 end
        end
      else
        _runLen=_hdr+1
        for _=1,_runLen do
...
`),l+=o;let f=t.encode(l),i="";for(let h=0;h<u;h++){let $=h===u-1,T=h+1,S=Se(f),s=Le(S),p=1e3+y(9e5),w=ve(s.ct,p);i=ke(w.out,s.k1,s.k2,s.k3,s.k4,s.rc4,w.seed,`vm${T}`,""),$||(f=t.encode(i))}let g=Me(i);return`--[[
...
`+g}function Ce(e,n={}){let t=W(e,n),r=re(t),a=n.vmDepth?Math.min(n.vmDepth,3):n.dualVm??!0?2:1;return{code:t,size:new TextEncoder().encode(t).length,originalSize:new TextEncoder().encode(e).length,entropy:r,layers:a,mode:a===2?"Dual Polymorphic Register VM":"Hardened Register VM"}}return de(Pe);})();
