// NOTE: the register-VM path uses only `base85Encode` / `compressToBase85`
// (pure TypeScript). The raw-deflate `compress`/`decompress` helpers below are
// kept for source compatibility with the upstream engine but load Node's zlib
// lazily so bundling this module for the browser never pulls in a Node builtin.
...
export function compress(input: Uint8Array): Uint8Array {
  // Lazy require so the browser bundle never resolves Node's zlib.
  const req = (globalThis as { require?: (m: string) => { deflateRawSync: (b: Uint8Array, o?: object) => Uint8Array } }).require;
  if (!req) throw new Error("raw deflate is only available on the server");
  return req("zlib").deflateRawSync(input, { level: 9 });
}

export function decompress(input: Uint8Array): Uint8Array {
  const req = (globalThis as { require?: (m: string) => { inflateRawSync: (b: Uint8Array) => Uint8Array } }).require;
  if (!req) throw new Error("raw inflate is only available on the server");
  return req("zlib").inflateRawSync(input);
}
