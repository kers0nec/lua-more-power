...
/**
 * Engine label stored against protected scripts and shown in the dashboard.
 * This is the LuaLamp VM-based obfuscator — the project's main/default engine.
 */
export const ENGINE_NAME = "LuaLamp v2";
...
export function obfuscateLuaWithOptions(source: string, options: ObfuscationOptions = {}): string {
  ...
  // `loaderVMDepth` (dashboard/API) and `vmDepth` (legacy) both select the
  // number of nested VM stages; the LuaLamp engine supports up to 3.
  const depth = options.vmDepth ?? options.loaderVMDepth;
  const layers = depth ? Math.min(depth, 3) : dualVm ? 2 : 1;
...
export function buildRobloxBufferLoader(payloadSource: string): string {
  const enc = new TextEncoder();
  const compressed = deflateSync(enc.encode(payloadSource));
...
export function analyzeObfuscation(
  ...
  return {
    ...
    mode:
      layers === 2
        ? "LuaLamp v2 · Dual Polymorphic Register VM"
        : "LuaLamp v2 · Hardened Register VM",
  };
}
