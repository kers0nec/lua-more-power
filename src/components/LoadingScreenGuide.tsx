    ...
    >
      ...
      <section className="mt-12">
        ...
        <div className="relative mt-6 flex w-full items-center justify-center overflow-hidden rounded-2xl border shadow-2xl" style={{ height: isMobile ? "300px" : "380px", maxWidth: isMobile ? "420px" : "100%", margin: isMobile ? "24px auto 0" : "24px 0 0", background: "radial-gradient(circle at 50% 30%, #152238 0%, #060911 100%)", borderColor: "var(--border-strong)" }}>
          ...
          <div className={`absolute flex flex-col justify-center rounded-xl p-3 pl-12 shadow-2xl ${position === "bottom-left" ? "bottom-4 left-4" : position === "bottom-center" ? "bottom-4 left-1/2 -translate-x-1/2" : position === "bottom-right" ? "bottom-4 right-4" : "top-4 left-1/2 -translate-x-1/2"}`} style={{ width: isMobile ? "85%" : "300px", height: "58px", background: preset.id === "gold" ? "#0d0c07" : preset.id === "neon" ? "#040e14" : preset.id === "clean" ? "#0e131d" : "#080e1a", border: `1px solid ${preset.id === "gold" ? "#f6c453" : preset.id === "neon" ? "#22d3ee" : preset.id === "clean" ? "#94a3b8" : "#0ea5e9"}` }}>
            <div className="absolute left-3.5 top-1/2 flex -translate-y-1/2 gap-1">{[0, 1, 2].map((item) => <span key={item} className="h-1.5 w-1.5 animate-ping rounded-full" style={{ background: preset.id === "gold" ? "#f6c453" : preset.id === "neon" ? "#22d3ee" : preset.id === "clean" ? "#e2e8f0" : "#0ea5e9", animationDelay: `${item * 0.2}s` }} />)}</div>
            <div className="flex flex-col"><span className="text-xs font-medium text-white">{title}</span><span className="mt-0.5 text-[10px] text-slate-400">{subtitle}</span><div className="mt-1.5 h-1 w-full overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, background: preset.id === "gold" ? "#f6c453" : preset.id === "neon" ? "#22d3ee" : preset.id === "clean" ? "#e2e8f0" : "#0ea5e9" }} /></div></div>
          </div>
        </div>
