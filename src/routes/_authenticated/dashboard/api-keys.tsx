  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <p className="mt-1" style={{ color: "var(--muted-foreground)" }}>
        Used by the Discord <code>/login</code> command and CLI/loader integrations.
      </p>
      ...
      <div className="card-blue p-5 mt-6 flex flex-wrap items-end gap-3">
        ...
        <button
          onClick={() => label && createMut.mutate()}
      ...
      {newKey && (
        <div className="mt-4 card-blue p-4" style={{ background: "rgba(52,211,153,0.12)" }}>
          ...
          <code className="block mt-2 font-mono text-sm p-2 rounded bg-[color:var(--input)]">
            {newKey}
          </code>
        </div>
      )}
          ...
          <div key={k.id} className="p-4 flex items-center gap-3">
            <div>
              ...
              <div className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                {k.prefix}•••••• · created {new Date(k.created_at).toLocaleString()}
              </div>
            </div>
